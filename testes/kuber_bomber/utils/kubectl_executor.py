...
class KubectlExecutor:
    ...
    def get_pods(self) -> List[str]:
        ...
        """
        if self.is_aws_mode and self.aws_config:
            # Modo AWS - usar aplicações específicas
            app_filter = ','.join(self.aws_config.get('applications', ['foo-app', 'bar-app', 'test-app']))
            result = self.execute_kubectl(['get', 'pods', '-A', '-o', 'jsonpath={.items[*].metadata.name}'])
        else:
            # Modo local
