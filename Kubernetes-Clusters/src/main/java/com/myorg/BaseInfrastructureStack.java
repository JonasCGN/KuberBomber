...
public class BaseInfrastructureStack extends Stack {
    ...
    public BaseInfrastructureStack(final Construct scope, final String id) throws IOException, InterruptedException {
        this(scope, id, null);
    }

    public BaseInfrastructureStack(final Construct scope, final String id, final StackProps props) throws IOException, InterruptedException {
        super(scope, id, props);

        // Verificar se AWS está habilitada através de variável de ambiente
        this.awsEnabled = Boolean.parseBoolean(System.getenv().getOrDefault("AWS_ENABLED", "true"));
        
        if (!awsEnabled) {
            System.out.println("=== MODO LOCAL ATIVADO ===");
            System.out.println("AWS_ENABLED=false - Recursos AWS não serão criados.");
            System.out.println("Para usar AWS, defina: export AWS_ENABLED=true");
            System.out.println("Para configurar ambiente local, execute: bash src/scripts/local_setup.sh");
            System.out.println("Para fazer deploy local, execute: bash src/scripts/local_deploy.sh");
            System.out.println("========================");
            return;
        }
        
        System.out.println("=== MODO AWS ATIVADO ===");
        System.out.println("Criando recursos na AWS...");
        ...
        String googleDrivePath="1CE1XAMKlaxfouWDHxzZdHOUXTCwpbcOg";
        String googleKeyPath="/home/iuresf/googleservices.json";
        // String vmPemKey="/home/iuresf/Downloads/awsacademy.pem";
        String vmPemKey="/home/iuresf/Downloads/vockey.pem";
        int numberOfWorkers=6;
        int numberOfClients=1;
        String useHPA="false";
        String barST="0.2";//rate=0.5
        String fooST="0.18";//rate=0.5
        String RABBITMQ_HOST="200.137.174.183";
        String RABBITMQ_PASS="asdfDersea!12";
        ...
        Vpc clientVPC = Vpc.Builder.create(this, "client_vpc")
                ...
                .enableDnsSupport(true)
                .enableDnsHostnames(true)
                .availabilityZones(new ArrayList<>(Collections.singletonList(availabilityZones.get(1))))
                .subnetConfiguration(List.of(
                        SubnetConfiguration.builder()
                                .subnetType(SubnetType.PUBLIC)  // Define as public
        ...
        ISecurityGroup ClientsecurityGroup = SecurityGroup.Builder.create(this, id + "_client_sg")
    ...
    // Método para verificar se está em modo AWS
    public boolean isAwsEnabled() {
        return this.awsEnabled;
    }


    public  CfnCluster createAnEMRCluter(Bucket bucket, String roleArn, String ubuntuAMIId, String vpcId, String keyName, ISecurityGroup securityGroup, CfnVPCGatewayAttachment gatewayAttachment, Vpc vpc, CfnRoute cfn, int instances){


//        String ubuntuAmiId = "ami-025b9fd66d61d093a"; // Replace with your Ubuntu AMI ID

        // Define the master instance group with the custom Ubuntu AMI
        CfnCluster.InstanceGroupConfigProperty masterInstanceGroup = CfnCluster.InstanceGroupConfigProperty.builder()
                .instanceCount(1)
...
//                .customAmiId(ubuntuAmiId)  // Use the custom Ubuntu AMI
        ...
        CfnCluster.InstanceGroupConfigProperty coreInstanceGroup = CfnCluster.InstanceGroupConfigProperty.builder()
...
//                .customAmiId(ubuntuAmiId)  // Use the custom Ubuntu AMI
        ...
        CfnCluster emrCluster = CfnCluster.Builder.create(this, "MyEmrCluster")
                ...
                .instances(CfnCluster.JobFlowInstancesConfigProperty.builder()
                        ...
                        .additionalMasterSecurityGroups(Arrays.asList(securityGroup.getSecurityGroupId()))
                        .additionalSlaveSecurityGroups(Arrays.asList(securityGroup.getSecurityGroupId()))
                        .build())
        ...
        CustomResource customResource = CustomResource.Builder.create(this, "CustomResource")
                ...
                .properties(Collections.singletonMap("ClusterId", emrCluster.getRef())) // Pass ClusterId to Lambda
                .build();
        ...
        Function cleanupLambda = Function.Builder.create(this, "CleanupLambda")
                ...
                .code(Code.fromAsset("src/scripts/customlambda"))  // Specify the path to your code directory
                .role(existingRole2)
                .build();
    ...
    public ApplicationLoadBalancer createLB(IVpc vpc,List<Instance> instances){
        ...
        ApplicationTargetGroup targetGroup = ApplicationTargetGroup.Builder.create(this, "HPATargetGroup")
                ...
                .healthCheck(HealthCheck.builder()
                        ...
                        .unhealthyThresholdCount(2)  // Number of consecutive failures to mark as unhealthy
    ...
    public Map<PublicSubnet,CfnRoute> createSubnets(int numberOfZones, Vpc vpc, CfnVPCGatewayAttachment cfnVPCGatewayAttachment){
    ...
    private Map<PublicSubnet,CfnSubnetRouteTableAssociation> createSubnets(Vpc vpc, String routeTableId) {
        ...
        for (int i = 0; i < 2; i++) {  // Create two subnets
            ...
            CfnSubnetRouteTableAssociation cfn=CfnSubnetRouteTableAssociation.Builder.create(this, "SubnetRouteTableAssociation_" + i)
    ...
    public Instance createCP(String name, Vpc vpc, String instanceAMI,ISecurityGroup securityGroup,ISubnet subnet,IKeyPair iKeyPair,InstanceType instanceType,Bucket bucket,String region,String exitingRoleArn, int numberOfWN, String useHPA,String barST,String fooST) throws IOException {

        String bootstrap = Files.readString(Paths.get("./src/scripts/nodes/controlPlane/bootstrap.sh"), Charset.defaultCharset());
        ...
        String userDataString = "echo '"+bootstrap+"' > /home/ubuntu/bootstrap.sh\n" + //add this file
                ...
                "aws s3 cp --recursive s3://"+bucket.getBucketName()+"/controlPlane/ /home/ubuntu/ \n" + //update
                "chmod +x /home/ubuntu/*.sh\n" + // add permission to execute the copied script
                "/home/ubuntu/controlPlaneInstall.sh\n" + //run the script
                "/home/ubuntu/afterInstall.sh "+bucket.getBucketName()+" "+numberOfWN+ " "+useHPA+ " "+barST+ " "+fooST+"\n"; //run the script

        Instance instance=createEc2Instance(name,vpc,instanceAMI,securityGroup,subnet,iKeyPair,userDataString,instanceType,region,exitingRoleArn);
    ...
    public Instance createWN(String name, Vpc vpc, String instanceAMI,ISecurityGroup securityGroup,ISubnet subnet,IKeyPair iKeyPair,InstanceType instanceType,Bucket bucket,String dcNum,String region,String exitingRoleArn) throws IOException {

        String bootstrap = Files.readString(Paths.get("./src/scripts/nodes/workerNode/bootstrap.sh"), Charset.defaultCharset());
    ...
    public Instance createClient(String name,
                                 ...
                                 String IPCP,
                                 String rabbitHost,
                                 String rabbitPass) throws IOException {
        ...
        String userDataString =  "echo '"+bootstrap+"' > /home/ubuntu/bootstrap.sh\n" + //add this file
                ...
                "chmod +x /home/ubuntu/*.sh\n" + // add permission to execute the copied script
                "/home/ubuntu/clientinstall.sh "+bucket.getBucketName()+" "+lbAddr+" "+googleDrivePath+" "+IPCP+" "+rabbitHost+" "+rabbitPass+"\n"; //run the script
...
//        System.out.println(userDataString);
    ...
    public Instance createEc2Instance(String name, Vpc vpc, String instanceAMI,ISecurityGroup securityGroup,ISubnet subnet,IKeyPair iKeyPair,String userDataString,InstanceType instanceType,String region,String exitingRoleArn) throws IOException {
        ...
        Instance engineEC2Instance = Instance.Builder.create(this,  name)
                ...
                .vpcSubnets(SubnetSelection.builder().subnets(new ArrayList<>(List.of(new ISubnet[]{subnet}))).build())
                .instanceType(instanceType)
                .keyPair(iKeyPair)
                .role(existingRole)
                .vpc(vpc)
...
// IP público da instância EC2
    ...
    public void addS3Asset(Bucket s3Bucket, String path, String existingRoleArn){
        ...
        Function uploadFilesLambda = Function.Builder.create(this, "RecursiveUploadLambda")
                .runtime(Runtime.PYTHON_3_9)
                .handler("upload_files.lambda_handler")  // Define the handler function in upload_files.py
        ...
        // Step 4: Create a CloudFormation custom resource to trigger the Lambda for upload
    ...
    public Bucket createBucket(String bucketName, String existingRoleArn) {
        ...
        Bucket s3Bucket = Bucket.Builder.create(this, bucketName)
                .removalPolicy(RemovalPolicy.DESTROY)  // Ensure the bucket is destroyed on stack deletion
                .build();
        ...
        // Create a CloudFormation custom resource that directly invokes the Lambda function
