...
class ConfigManager:
    ...
    def __init__(self, config_file: Optional[str] = None, aws_mode: bool = False, aws_config: Optional[dict] = None):
        """
        ...
        Args:
            ...
            aws_config: Configuração AWS para detecção correta de contexto
        """
        ...
        # No modo AWS, não detectar contexto kubectl local
        if not aws_mode:
            self._detect_and_set_context()
        else:
            # No modo AWS, usar contexto especial e detectar via AWS config
            self._detect_and_set_context(aws_config=aws_config)
    
    def _detect_and_set_context(self, aws_config: Optional[dict] = None):
        """Detecta e define o contexto kubectl automaticamente."""
        context = self._detect_current_context(aws_config)
        if context:
            self.config.context = context
    
    def _detect_current_context(self, aws_config: Optional[dict] = None) -> str:
        """
        Detecta o contexto atual do kubectl automaticamente.
        Para modo AWS, usa contexto via SSH remoto.
        
        Args:
            aws_config: Configuração AWS se disponível
        
        Returns:
            Nome do contexto atual ou fallback
        """
        # Se está no modo AWS, usar contexto remoto
        if aws_config:
            print(f"🔍 Contexto detectado automaticamente: aws-remote (SSH: {aws_config.get('ssh_host', 'unknown')})")
            return "aws-remote"  # Contexto especial para AWS
        ...
        try:
            ...
            if current_context:
                print(f"🔍 Contexto detectado automaticamente: {current_context} (LOCAL)")
...
def get_config(aws_mode: bool = False, aws_config: Optional[dict] = None) -> ReliabilityConfig:
    """
    ...
    if DEFAULT_CONFIG is None or getattr(DEFAULT_CONFIG, 'aws_mode', False) != aws_mode:
        DEFAULT_CONFIG = ConfigManager(aws_mode=aws_mode, aws_config=aws_config)
