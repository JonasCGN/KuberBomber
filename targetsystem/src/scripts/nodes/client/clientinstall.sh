...
run_dt_container(){
  ...
  export WINDOW_SECONDS=60 #always > 60
  export DATA_TIME_FILTER_LAST=10
  export DATA_TIME_FILTER_LAST=10
  export STEP_SECONDS=1
  export SLA=2.0
  export DELAY_BETWEEN_RUN=10
  export REMOVE_WINDOW=10
...
install_client(){
  ...
  echo "copy kubernetes files"
    ...
    if [ "${use_autoscaling}" = "false" ]; then
      echo "run docker"
      install_docker

      wait_copy_from_s3 "$BUCKET" "kubeconfig.conf" "/home/ubuntu/kubeconfig.conf"

      run_dt_container "$CP_PUBLIC_ADDR" "$RABBITMQ_HOST" "$RABBITMQ_PASS" "$RABBITMQ_PORT"
  fi
